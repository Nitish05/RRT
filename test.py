...
def sample_ellipse(best_cost, c_min, x_center, c_max):
    if best_cost < float('inf'):
        L = np.array([[c_max / 2, 0], [0, np.sqrt(best_cost**2 - c_min**2) / 2]])
        x_ball = random.uniform(0, 1) * 2 * np.pi
        r_ball = (random.uniform(0, 1) ** 0.5) * c_max / 2
        point = np.dot(L, np.array([np.cos(x_ball), np.sin(x_ball)]) * r_ball)
        return tuple(map(int, point + x_center))
    else:
...
def informed_RRT_star(start, goal, iterations=5000, search_radius=20):
    ...
    for _ in range(iterations):
        if random.randint(0, 100) > 5:
            rand_point = sample_ellipse(best_cost, c_min, x_center, canvas_width)
        else:
            rand_point = goal
        ...
        if new_node:
            ...
            if distance(new_node, goal) < 10:
                path_cost = cost(tree, new_node) + distance(new_node, goal)
                if goal_node is None or path_cost < best_cost:
                    best_cost = path_cost
                    goal_node = new_node
...
out = cv2.VideoWriter('RRT_star_informed.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (canvas_width, canvas_height))

start = (50, 100)
goal = (550, 100)
cv2.circle(canvas, start, robo_radius, (0, 255, 0), -1)
cv2.circle(canvas, goal, robo_radius, (0, 0, 255), -1)
for i in range(30):
    out.write(canvas)
