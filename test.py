...
def choose_parent(tree, new_node, near_nodes, ancestry_depth=2):
    best_parent = None
    best_cost = float('inf')
    # Check direct connections and ancestry for better paths
    for node in near_nodes:
        current_node = node
        for _ in range(ancestry_depth):  # Look up to 'ancestry_depth' ancestors for a potential better path
            if tree[current_node] is None:
                break
            if is_free(*new_node) and is_free(*current_node) and cost(tree, current_node) + distance(current_node, new_node) < best_cost:
                best_parent = current_node
                best_cost = cost(tree, current_node) + distance(current_node, new_node)
...
def rewire(tree, new_node, near_nodes, ancestry_depth=2):
    for node in near_nodes:
        current_node = node
        for _ in range(ancestry_depth):  # Consider ancestors for potential rewiring
            if tree[current_node] is None:
                break
            if is_free(*new_node) and is_free(*current_node) and cost(tree, new_node) + distance(new_node, current_node) < cost(tree, current_node):
                tree[current_node] = new_node
                cv2.line(canvas, new_node, current_node, path_color, 1)
            current_node = tree[current_node]
    return tree

def rewire_goal(tree, goal_node, near_nodes, ancestry_depth=2):
    for node in near_nodes:
        current_node = node
        for _ in range(ancestry_depth):  # Consider ancestors for potential better connection to the goal
            if tree[current_node] is None:
                break
            if is_free(*goal_node) and is_free(*current_node) and cost(tree, goal_node) + distance(goal_node, current_node) < cost(tree, current_node):
                tree[current_node] = goal_node
                cv2.line(canvas, goal_node, current_node, path_color, 1)
            current_node = tree[current_node]
    return tree
